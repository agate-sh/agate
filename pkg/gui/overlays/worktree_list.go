...
func (d itemDelegate) Render(w io.Writer, m list.Model, index int, listItem list.Item) {
	...
	case "main_repo":
		...
		truncatedPath := common.TruncatePathFromLeft(item.RepoPath, availableWidth)
	...
	if _, err := fmt.Fprint(w, content); err != nil {
		...
		debug.DebugLog("Failed to write worktree content to buffer: %v", err)
	}
}
...
func NewWorktreeList(worktreeManager *git.WorktreeManager) *WorktreeList {
	...
	if err := wl.Refresh(); err != nil {
		debug.DebugLog("Failed to refresh worktree list during initialization: %v", err)
