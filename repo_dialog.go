...
func (d *RepoDialog) Init() tea.Cmd {
	DebugLog("RepoDialog.Init() called")
	// Start the repository discovery and fzf selection process
	d.searching = true
	d.startTime = time.Now()
	DebugLog("Starting repository search with spinner and StartRepoSelection()")
	return tea.Batch(
		d.spinner.Tick,
		StartRepoSelection(),
	)
...
func StartRepoSelection() tea.Cmd {
	return func() tea.Msg {
		// Add debug output
		DebugLog("Starting repository discovery...")

		// First discover repositories
		repos, err := discoverRepositories()
		if err != nil {
			DebugLog("Repository discovery failed: %v", err)
			return RepoSelectedMsg{Error: err.Error()}
		}

		DebugLog("Found %d repositories, starting fzf...", len(repos))

		// Then run fzf selection
		selected, err := runFzfSelection(repos)
		if err != nil {
			DebugLog("fzf selection failed: %v", err)
			return RepoSelectedMsg{Error: err.Error()}
		}

		if selected == "" {
			DebugLog("User cancelled fzf selection")
			// User cancelled
			return RepoSelectionCancelledMsg{}
		}

		DebugLog("User selected: %s", selected)
